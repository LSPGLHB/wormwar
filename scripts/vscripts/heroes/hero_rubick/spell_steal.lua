...
function GetLastSpell(keys)
	...
	for i=0, 15 do
		local current_ability = target:GetAbilityByIndex(i) --以index获取技能
		if current_ability ~= nil then
			-- If the ability is toggleable, checks if it was turned on 如果技能可以切换。检测其是否已使用
			if current_ability:IsToggle() then
				...
				--通过检查冷却时间是否等于完全冷却时间来查找导致事件触发器的能力
				local cd = current_ability:GetCooldownTimeRemaining()  --获取剩余冷却时间
				local full_cd = current_ability:GetCooldown(current_ability:GetLevel()-1)
				-- There is a delay after the ability cast event and before the ability goes on cooldown在技能施放事件之后和技能冷却之前有一个延迟
				-- If the ability is on cooldown and the cooldown is within a small buffer of the full cooldown如果该技能处于冷却状态，并且冷却时间在完全冷却的小缓冲区内
				-- Set this as the target's most recently cast ability 将其设置为目标最近的施法能力
